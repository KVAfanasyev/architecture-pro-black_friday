...
app = FastAPI(title="MongoDB Sharding API", version="1.0.0")
...
async def health_check():
    ...
    return HealthCheck(
        status="healthy" if mongodb_status == "healthy" and redis_status in ["healthy", "not configured"] else "degraded",
...
async def root():
    ...
    cache_enabled = False
    redis_status = await check_redis_health()
    if REDIS_URL and redis_status == "healthy":
...
@cache(expire=30)  # Кэшируем на 30 секунд
...
@cache(expire=60)  # Кэшируем на 1 минуту
async def list_users(collection_name: str):
    """
    List all of the user data in the database.
    The response is unpaginated and limited to 1000 results.
    """
    # Имитация задержки для демонстрации преимуществ кэширования
    time.sleep(1)
    collection = db.get_collection(collection_name)
...
@cache(expire=120)  # Кэшируем на 2 минуты для отдельных пользователей
...
async def create_user(collection_name: str, user: UserModel = Body(...)):
    ...
    # Инвалидируем кэш для списка пользователей
    if REDIS_URL:
        try:
            redis = aioredis.from_url(REDIS_URL, encoding="utf8", decode_responses=True)
            await redis.delete(f"api:cache:list_users:{collection_name}")
            await redis.close()
        except Exception as e:
...
async def clear_cache():
    ...
    if REDIS_URL:
        ...
        raise HTTPException(status_code=400, detail="Redis cache is not configured")